  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-[#0F172A] tracking-tight">Knowledge Base Library</h1>
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-sm flex flex-wrap gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition
                ${selectedCategory === '' ? 'bg-blue-50 text-[#2563EB]' : 'text-slate-650 hover:bg-slate-50'}`}
            >
              All Topics
            </button>
            {categories.map(cat => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition
                  ${selectedCategory === cat ? 'bg-blue-50 text-[#2563EB]' : 'text-slate-650 hover:bg-slate-50'}`}
              >
                {cat}
              </button>
            ))}
              ...
              articles.map(art => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 hover:bg-slate-50/50 cursor-pointer transition flex items-start justify-between gap-4 ${active ? 'bg-blue-50/30 border-l-4 border-l-[#2563EB]' : ''}`}
                  >
                    <div className="space-y-1">
                      <h4 className="font-bold text-[#0F172A] text-xs leading-snug">{art.title}</h4>
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-6 space-y-6 min-h-[500px] flex flex-col justify-between animate-scale-up">
              ...
              {loadingDetail ? (
                ...
                <>
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start justify-between gap-4 border-b border-slate-50 pb-4">
                      <div>
                        ...
                        <h2 className="text-base font-bold text-[#0F172A] tracking-tight">{selectedArticle?.title}</h2>
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50 transition shrink-0"
                      >
                        Edit Article
                      </button>
                    </div>
                  ...
                  {/* Ratings panel */}
                  <div className="border-t border-slate-100 pt-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-8">
                    <div>
                      <p className="text-[11px] font-bold text-slate-450 uppercase tracking-wide">Was this article helpful?</p>
                      <div className="flex gap-2 mt-2">
                        <button
                          onClick={() => handleRate(true)}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 hover:bg-slate-100 rounded-xl text-xs font-bold text-slate-650 border border-slate-200 transition"
                        >
