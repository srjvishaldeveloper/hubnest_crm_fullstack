...
import {
  MessageSquare, Plus, Search, Loader2, RefreshCw,
  Edit2, Copy, Trash2, X, Users, Calendar, ChevronDown, CheckCircle,
} from 'lucide-react';
...
function CreateModal({ onClose, onCreated }: ModalProps) {
  const [form, setForm] = useState({
    name: '', template: '', audience: '', scheduled_at: '',
  });
      ...
      <div className="w-full max-w-lg bg-white dark:bg-[#161616] rounded-2xl border border-slate-200/60 dark:border-[#1f1f1f] shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-[#1f1f1f]">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-slate-900 dark:text-[#ededed]">Create WhatsApp Campaign</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-[#333] bg-white dark:bg-[#161616] text-slate-900 dark:text-[#ededed] placeholder:text-slate-400 dark:placeholder:text-[#555] focus:outline-none focus:ring-2 focus:ring-green-500/40"
            />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-500 dark:text-[#a3a3a3]">WhatsApp Template</label>
            <div className="relative">
              <select
                value={form.template} onChange={e => set('template', e.target.value)}
                className="w-full appearance-none pl-3 pr-8 py-2 text-sm rounded-lg border border-slate-200 dark:border-[#333] bg-white dark:bg-[#161616] text-slate-900 dark:text-[#ededed] focus:outline-none focus:ring-2 focus:ring-green-500/40"
              >
                <option value="">Select a template…</option>
                {WA_TEMPLATES.map(t => <option key={t} value={t}>{t.replace(/_/g, ' ')}</option>)}
              </select>
              <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" />
            </div>
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-500 dark:text-[#a3a3a3]">Audience Segment</label>
            <div className="relative">
              <select
              ...
              >
                ...
                <option value="inactive_60d">Inactive 60 Days</option>
              </select>
              <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" />
            </div>
          </div>
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-[#333] bg-white dark:bg-[#161616] text-slate-900 dark:text-[#ededed] focus:outline-none focus:ring-2 focus:ring-green-500/40"
            />
          </div>
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={saving}
              className="flex items-center gap-2 px-4 py-2 text-sm rounded-lg bg-green-600 hover:bg-green-700 text-white font-medium transition disabled:opacity-60">
...
export default function WhatsAppCampaignsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await api.get('/campaigns?type=WhatsApp');
  ...
  const deliveredRate = totalSent > 0 ? ((totalDelivered / totalSent) * 100).toFixed(1) : '—';
  const readRate = totalDelivered > 0 ? ((totalRead / totalDelivered) * 100).toFixed(1) : '—';
  const replyRate = totalDelivered > 0 ? ((totalReplied / totalDelivered) * 100).toFixed(1) : '—';

  const statCards = [
    { label: 'Messages Sent', value: loading ? null : totalSent.toLocaleString() },
    { label: 'Delivered Rate', value: loading ? null : (totalSent > 0 ? `${deliveredRate}%` : '—') },
    { label: 'Read Rate', value: loading ? null : (totalDelivered > 0 ? `${readRate}%` : '—') },
    { label: 'Reply Rate', value: loading ? null : (totalDelivered > 0 ? `${replyRate}%` : '—') },
  ];
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-slate-900 dark:text-[#ededed]">WhatsApp Campaigns</h1>
          <p className="text-sm text-slate-500 dark:text-[#a3a3a3] mt-0.5">Send targeted messages via WhatsApp Business API</p>
        </div>
        <button onClick={() => setShowModal(true)}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-medium transition shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search campaigns…"
            className="w-full pl-9 pr-3 py-2 text-sm rounded-xl border border-slate-200 dark:border-[#333] bg-white dark:bg-[#161616] text-slate-900 dark:text-[#ededed] placeholder:text-slate-400 dark:placeholder:text-[#555] focus:outline-none focus:ring-2 focus:ring-green-500/40" />
        </div>
        <div className="relative">
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            className="appearance-none pl-3 pr-8 py-2 text-sm rounded-xl border border-slate-200 dark:border-[#333] bg-white dark:bg-[#161616] text-slate-900 dark:text-[#ededed] focus:outline-none focus:ring-2 focus:ring-green-500/40">
      ...
      <div className="bg-white dark:bg-[#161616] rounded-2xl border border-slate-200/60 dark:border-[#1f1f1f] shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center">
            ...
            <button onClick={fetchData} className="mt-3 text-sm text-green-600 hover:underline">Retry</button>
          </div>
        ) : filtered.length === 0 ? (
          <div className="p-16 text-center">
            <MessageSquare className="w-10 h-10 text-slate-300 dark:text-[#333] mx-auto mb-3" />
            <p className="text-sm font-medium text-slate-900 dark:text-[#ededed]">No WhatsApp campaigns yet</p>
            <p className="text-xs text-slate-500 dark:text-[#a3a3a3] mt-1">Create your first WhatsApp campaign to get started</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-[#1f1f1f]">
                  {['Campaign', 'Template', 'Audience', 'Status', 'Sent', 'Delivered', 'Read', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-[#1f1f1f]">
                {filtered.map(c => (
                  <tr key={c.id} className="hover:bg-slate-50/50 dark:hover:bg-[#1a1a1a] transition">
                    ...
                    <td className="px-4 py-3 text-slate-500 dark:text-[#a3a3a3] text-xs">
                      {c.template_name ? c.template_name.replace(/_/g, ' ') : '—'}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-slate-700 dark:text-[#ededed] text-xs font-medium">{c.sent_count?.toLocaleString() ?? '—'}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-slate-700 dark:text-[#ededed] text-xs">{c.read_count?.toLocaleString() ?? '—'}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
