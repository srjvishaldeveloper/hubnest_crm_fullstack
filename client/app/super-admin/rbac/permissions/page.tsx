...
interface PermissionMatrix {
  resources: string[];
  roles: string[];
  matrix: Record<string, Record<string, { read: boolean; write: boolean; delete: boolean }>>;
}
...
export default function PermissionsPage() {
  ...
  function toggle(resource: string, role: string, perm: 'read' | 'write' | 'delete') {
    if (!local) return;
    setLocal(prev => {
      if (!prev) return prev;
      const copy = JSON.parse(JSON.stringify(prev)) as PermissionMatrix;
      if (!copy.matrix[resource]) copy.matrix[resource] = {};
      if (!copy.matrix[resource][role]) copy.matrix[resource][role] = { read: false, write: false, delete: false };
            ...
            <thead>
              <tr className="bg-slate-50 dark:bg-[#161616] border-b border-slate-200">
                ...
                {local.roles.map(role => (
                  <th key={role} colSpan={3} className="text-center px-2 py-3.5 text-xs font-semibold text-slate-600 border-l border-slate-200">
                    {role}
                  </th>
                ))}
              </tr>
