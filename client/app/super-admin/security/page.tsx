...
export default function SuperAdminSecurityPage() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-[#0F172A]">Platform Security Control</h1>
        <p className="text-xs text-[#64748B] mt-0.5">Control global multi-tenant access firewalls, login rate limits, and 2FA policies</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200/60 p-5 space-y-4">
            <h3 className="text-xs font-bold text-[#0F172A] uppercase flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-emerald-500" /> Platform Shield Policy</h3>
            {[
              ...
              <div key={idx} className="flex justify-between items-center p-3.5 border border-slate-100 rounded-xl">
                <div>
                  <span className="text-xs font-bold text-[#0F172A]">{p.label}</span>
                  <p className="text-[10px] text-slate-500 mt-0.5">{p.desc}</p>
                </div>
                <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-xl">{p.val}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 space-y-4">
          <h3 className="text-xs font-bold text-[#0F172A] uppercase flex items-center gap-1.5"><ShieldAlert className="w-4 h-4 text-rose-500" /> Security Logs</h3>
          <div className="space-y-3">
            {[
              ...
              <div key={idx} className="text-xs border-b border-slate-50 pb-2">
                <p className="font-medium text-[#0F172A]">{log.msg}</p>
